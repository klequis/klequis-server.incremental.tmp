...
import { transactionFields as tFields, actionTypes } from 'db/constants'
...
import { wdRules, wdTransactions } from '../appWords'
...
const getRules = async (ruleId = '') => {
  if (isNilOrEmpty(ruleId)) {
    return find({ collection: wdRules })
  } else {
    return findById({ collection: wdRules, id: ruleId })
  }
}
...
const runRules = async (ruleId) => {
  ...
  //   rules = passedInRules
  // } else {
  //   const allRules = await find(wdRules, {})
  //   rules = allRules
  // }
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    const f = await find({ collection: wdTransactions, filter: filter })
      ...
      switch (action.actionType) {
        ...
        case actionTypes.strip:
          for (let j = 0; j < f.length; j++) {
            ...
            await findOneAndUpdate({
              collection: wdTransactions,
              filter: { _id: doc._id },
              update: _createStripUpdate(action, doc, rule)
            })
          }
        ...
        case actionTypes.categorize:
          await updateMany({
            collection: wdTransactions,
            filter: filter,
            update: _createCategorizeUpdate(action, rule)
          })
